...
| Hook resolver thumbnail (blob OU signed URL) | [src/features/chat/lib/useAttachmentThumb.ts](./src/features/chat/lib/useAttachmentThumb.ts) |
...
| Pull (cursors par table) | [src/shared/sync/pull.ts](./src/shared/sync/pull.ts) |
...
### 🟠 Pipeline pièces jointes (en cours)

- **Compteur `VisitAttachmentSyncStatus` instable** — voir plan correctif `claude/lifecycle-state-machine` (champ explicite `attachment_status` au lieu de jointures inter-tables).
- **Thumbnails non visibles cross-device** — `useAttachmentThumb` n'expose pas l'erreur signed URL → loading infini si Storage 404. **Fix** : status détaillé + `<img onError>`.
- **`describe_media` silencieux** — pas de surface UI pour les échecs (clé manquante, quota, 5xx). **Fix** : retry button + toast.
- **Pas de dédup serveur de `describe_media`** — 2 devices peuvent enqueue le même job → double consommation Gemini. **Fix** : UNIQUE constraint sur `attachment_ai_descriptions(attachment_id, mode)` ou dédup handler.
