...
describe("Dexie schema — smoke", () => {
  it("ouvre la DB en v5 et expose les 10 tables (incl. llm_extractions, attachment_ai_descriptions)", async () => {
    const db = getDb();
    await db.open();
...
describe("Idempotence client_id (équivalent ON CONFLICT DO NOTHING)", () => {
  it("upsertVisitFromRemote avec même (user_id, client_id) ne dédouble pas", async () => {
    const row: VisitRow = {
      ...
      address: null,
      mission_type: null,
      mission_type_other: null,
      building_type: null,
      building_type_other: null,
      tertiaire_subtype: null,
      tertiaire_subtype_other: null,
      visit_started_at: "2026-01-01T00:00:00.000Z",
      gps_lat: null,
      gps_lng: null,
      gps_accuracy_m: null,
