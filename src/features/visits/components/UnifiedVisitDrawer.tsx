/**
 * VTU — Itération 13 : Drawer unifié de la VT.
 *
 * Un seul panneau qui regroupe TOUT ce qu'on peut consulter sur une VT,
 * organisé en 4 onglets :
 *
 *   1. Synthèse   — fiche bâtiment lisible (it. 12)
 *   2. JSON       — état brut + mode "À traiter" (it. 11)
 *   3. Photos     — grille type Telegram, regroupée par section
 *   4. Actions IA — historique des propositions IA, filtrable
 *
 * UX :
 *   - Sur mobile : slide-up plein écran depuis le bas
 *   - Sur desktop : panneau side-right large
 *   - Header sticky avec barre de progression globale (% champs validés)
 *   - Onglet courant mémorisé entre ouvertures (localStorage)
 *
 * Doctrine : "1 seul endroit pour tout savoir" — le chat reste le mode
 * capture, ce drawer reste le mode revue.
 */
...
export type DrawerTab = "summary" | "json" | "photos" | "ai_actions";
...
interface UnifiedVisitDrawerProps {
  ...
  /** Onglet à pré-sélectionner. Override le tab mémorisé pour cette ouverture. */
  initialTab?: DrawerTab;
  /** Pour l'onglet JSON : pousse en mode "À traiter" à l'ouverture. */
  jsonInitialMode?: "tree" | "todo";
}

const STORAGE_KEY = "vtu:visit-drawer:tab";

const TABS: Array<{ key: DrawerTab; label: string; Icon: typeof Braces }> = [
...
function readStoredTab(): DrawerTab {
  ...
  try {
    ...
    if (v === "summary" || v === "json" || v === "photos" || v === "ai_actions") {
      return v;
    }
  } catch {
    /* ignore */
...
function persistTab(tab: DrawerTab) {
...
export function UnifiedVisitDrawer({
  ...
  // Quand le parent demande un onglet précis à l'ouverture
  useEffect(() => {
    if (!open) return;
    if (initialTab) {
      setTab(initialTab);
  ...
  // ----- Données pour la barre de progression globale -----
      ...
      >
        ...
        <SheetHeader className="border-b border-border/60 bg-card/60 p-3 text-left">
          ...
          <nav
            ...
            role="tablist"
            aria-label="Sections du drawer"
          >
            {TABS.map(({ key, label, Icon }) => {
              const active = tab === key;
              const badge =
                key === "ai_actions"
                  ? globals.aiUnvalidated + conflictsCount
                  : key === "photos"
                    ? media.length
                    : 0;
              return (
